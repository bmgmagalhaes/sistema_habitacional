...
class Beneficiario(models.Model):
    ...
    cpf = models.CharField(
        ...
        unique=True,
        validators=[
            RegexValidator(regex=r"^\d{11}$", message="CPF deve conter 11 dígitos.")
        ],
        verbose_name="CPF",
    )
    ...
    nis = models.CharField(
        ...
        unique=True,
        validators=[
            RegexValidator(regex=r"^\d{11}$", message="NIS deve conter 11 dígitos.")
        ],
        verbose_name="NIS",
    )
    ...
    possui_doenca_rara = models.BooleanField(
        default=False, verbose_name="Possui doença rara?"
    )
    mulher_responsavel_familia = models.BooleanField(
        default=False, verbose_name="Mulher responsável pela família?"
    )
    ...
    posicao_familiar = models.CharField(
        max_length=20, choices=POSICAO_FAMILIAR_CHOICES, verbose_name="Posição familiar"
    )
    ...
    numero_pessoas_familia = models.PositiveIntegerField(
        default=1, verbose_name="Número de pessoas na família"
    )
    renda_familiar = models.DecimalField(max_digits=10, decimal_places=2)

    recebe_bolsa_familia = models.BooleanField(
        default=False, verbose_name="Recebe Bolsa Família?"
    )
    deficiencia_na_familia = models.BooleanField(
        default=False, verbose_name="Há pessoa com deficiência na família?"
    )
    doenca_rara_na_familia = models.BooleanField(
        default=False,
        verbose_name="Há pessoa com câncer ou doença rara crônica e degenerativa na família?",
    )
    recebe_bpc = models.BooleanField(default=False, verbose_name="Recebe BPC?")
    pessoa_negra_na_familia = models.BooleanField(
        default=False, verbose_name="Há pessoa negra na família?"
    )
    mulheres_violencia_domestica = models.BooleanField(
        default=False,
        verbose_name="Mulheres vítima de violência doméstica ou familiar?",
    )
    povos_tradicionais_quilombolas = models.BooleanField(
        default=False,
        verbose_name="Há integrantes de povos tradicionais ou quilombolas na família?",
    )
    idosos_na_familia = models.BooleanField(
        default=False, verbose_name="Há idosos na família?"
    )
    beneficiario_contrato_rescindido = models.BooleanField(
        default=False,
        verbose_name="Beneficiário cujo contrato foi distrtado ou rescindido involuntariamente?",
    )
    microcefalia_na_familia = models.BooleanField(
        default=False, verbose_name="Há pessoa com microcefalia na família?"
    )

    filhos_0_6 = models.PositiveIntegerField(
        default=0, verbose_name="Filhos(a) de 0 a 6 anos"
    )
    filhos_7_11 = models.PositiveIntegerField(
        default=0, verbose_name="Filhos(a) de 7 a 11 anos"
    )
    filhos_12_18 = models.PositiveIntegerField(
        default=0, verbose_name="Filhos(a) de 12 a 18 anos"
    )
    ...
    residente_area_risco = models.BooleanField(
        default=False, verbose_name="Residente em área de risco?"
    )
    situacao_rua = models.BooleanField(
        default=False, verbose_name="Pessoa em situação de rua?"
    )
    ...
    situacao_cadastral = models.CharField(
        max_length=20, choices=SITUACAO_CADASTRAL_CHOICES, default="em_analise"
    )
...
class CriterioPontuacao(models.Model):
    ...
    fundamentacao_legal = models.CharField(
        max_length=255, blank=True, verbose_name="Fundamentação legal"
    )
